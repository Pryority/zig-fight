...
pub const Camera = struct {
    ...
    pub fn init(player: Player, width: i32, height: i32) Camera {
        return Camera{ .camera = c.Camera2D{ .offset = c.Vector2{ .x = @floatFromInt(@divExact(c.GetScreenWidth(), 2)), .y = @floatFromInt(@divExact(c.GetScreenHeight(), 2)) }, .target = player.position, .rotation = 0.0, .zoom = 1.0 }, .player = player, .width = width, .height = height };
    }
